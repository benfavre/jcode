...
use jcode_operator_backend::platform_contract::{
    ...
    PlatformText, ResourceAuthority, ResourceCoordinate, ResourceId, ResourceKind, ResourceRecord,
};
...
async fn apply_command(
    ...
    match command {
        ...
        AvailableCommand::Steer => open_composer(command, state, composer),
...
async fn execute_confirmed(
    ...
    let current_revision = state
        ...
        .or_else(|| {
            (pending.target.kind == ResourceKind::ControlLease)
                .then(|| {
                    state.panes.iter().find_map(|pane| {
                        pane.control.as_ref().and_then(|lease| {
                            (lease.id.as_str() == pending.target.id.as_str())
...
impl AvailableCommand {
    const fn label(self) -> &'static str {
        match self {
            ...
            Self::Steer => "Steer active turn with focused control lease",
...
fn available_commands(state: &CockpitState) -> Vec<AvailableCommand> {
    ...
    if !state.panes.is_empty() {
        ...
        if supports_action(PlatformAction::Steer)
            && state
                .panes
                .get(state.focused_pane)
                .is_some_and(|pane| pane.control.is_some())
        {
            commands.push(AvailableCommand::Steer);
        }
    ...
    fn into_pending(self) -> Option<PendingAction> {
        ...
        let (title, consequence) = match self.action {
            ...
            PlatformAction::Steer => (
                "Steer active provider turn",
                "The text is injected only through this exact unexpired exclusive control lease.",
            ),
            _ => return None,
        };
        Some(PendingAction {
...
fn open_composer(
    ...
    if command == AvailableCommand::Steer {
        if !state.overview.actions.contains(&PlatformAction::Steer) {
            state.status = String::from("server does not advertise active-turn steering");
            return;
        }
        let Some(lease) = state
            .panes
            .get(state.focused_pane)
            .and_then(|pane| pane.control.as_ref())
        else {
            state.status = String::from("focused pane holds no active control lease");
            return;
        };
        let target = ResourceCoordinate::new(
            ResourceAuthority::Automonique,
            ResourceKind::ControlLease,
            match ResourceId::new(lease.id.as_str()) {
                Ok(id) => id,
                Err(_) => {
                    state.status = String::from("control lease identity is invalid");
                    return;
                }
            },
        );
        let target_label = coordinate_key(&target);
