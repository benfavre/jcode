...
impl OvernightRunStatus {
    ...
    ///
    /// These statuses are claims that a process is actively driving the run,
    /// not standalone facts. `Running` advances only when the owning process
    /// takes another coordinator turn, and `CancelRequested` is cleared only
    /// when that same process observes the request and marks the run completed.
    /// Neither can make progress on its own, so a manifest left in either state
    /// by a crash, a kill, or a reboot is stale rather than live, and the
    /// recorded pid is what settles which half of the contradiction to believe.
